...
def get_comments(post_id):
    return db.query("""SELECT u.username, c.content, c.rating, c.created_at, c.edited_at, c.user_id, c.id
...
def create_post(parameters):
    db.execute("INSERT INTO posts (user_id, title, release_year, movie_hours, movie_minutes, rating, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)", parameters)
...
def add_comment(post_id, user_id, content, rating, edited_at):
    db.execute("INSERT INTO comments (post_id, user_id, content, rating, created_at) VALUES (?, ?, ?, ?, ?)", [post_id, user_id, content, rating, edited_at])
